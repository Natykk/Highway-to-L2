 ...
 *  
*/
void changement(t_niv * niv, t_dir move,entite_t* Perso, t_pos * posSalle, t_salle * map){
    if(move==HAUT){
        posSalle->y--;
        Perso->y=DIM_SALLE-2;
 ...
 * 
*/
void mouvement(t_salle * map, t_dir move, entite_t* Perso, t_pos * posSalle, t_niv * niv){
    int x = Perso->x;
    int y = Perso->y;
    if(move == HAUT){
        if(map->dim[x][y-1] == PORTE){ // Si la case du haut est une porte
            changement(niv, move, Perso, posSalle, map); 
        }
        if(map->dim[x][y-1] != MUR && map->dim[x][y-1] != PORTE && map->dim[x][y-1] != OBSTACLE){ // Si la case du haut n'est pas un mur
            Perso->x = x; // On met à jour la position du perso
            Perso->y = y-1;
            map->dim[Perso->x][Perso->y] = PERSO;  //map[(y-1)*MAP_WIDTH+x] = 5; // On met le perso sur la case du haut
...
/**
 * \fn SDL_Texture* charge_tex(SDL_Renderer *renderer,char *path,int bmp_flag)
...
*/
int main() {
    t_niv * niv = malloc(sizeof(t_niv)); // On crée le niveau
    genererNiv(niv);    // On génère le niveau

    SDL_Rect PersRect; 
    int frame;

    entite_t * Perso = malloc(sizeof(entite_t));
    creer_personnage(Perso);   // On crée le perso 
    ...
    SDL_Texture* Mur = charge_tex(renderer,"brick.png",0); // On charge les textures
    ...
    Perso->x=DIM_SALLE/2;// Position du perso au millieu de la salle
    Perso->y=DIM_SALLE/2;
    posSalle = trouverSalle(1, &niv->etages[0]);

    transfert(niv->etages[0].etage[posSalle.x][posSalle.y], &map); // On transfère la salle dans la matrice
    map.num_salle=niv->etages[0].etage[posSalle.x][posSalle.y].num_salle; // On récupère le numéro de la salle

    map.dim[Perso->x][Perso->y] = PERSO; // On place le perso dans la salle
            ...
            case SDL_KEYDOWN:
                    switch (event.key.keysym.scancode){ // On récupère le code de la touche
                        ...
                        case SDL_SCANCODE_UP:
                            move=HAUT;
                            mouvement(&map, move, Perso, &posSalle, niv); // On déplace le perso
                            break;
    ...
    detruire_entitee(Perso); // On détruit la structure perso dans la mémoire
    detruireNiv(&niv); // On libère la mémoire du niveau
